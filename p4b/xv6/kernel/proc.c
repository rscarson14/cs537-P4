...
clone(void)
{
  ...
  // Need to grab the stack ptr
  void *nStack;
  if(argptr(0, (void *) &nStack, sizeof(nStack)) < 0){
    return -1;
  }

  // Hopefully the userspace process takes care of this, but error checking just in case
  if((uint) nStack % PGSIZE != 0){
    return -1;
  }

  cprintf("thread stack is at %x to %x\n", nStack, (uint)nStack + PGSIZE -1);

  // Allocate process.
  if((np = allocproc()) == 0)
    return -1;

  /* Not needed anymore, simply point to the same page dir
  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
